...
scratch_destriped=${scratch_directory}"lightsheet/raw/"
...
do
TMP=$(echo $i)
sbatch --job-name=stitch_files --mem=300G --partition=scu-gpu --gres=gpu:2 --mail-type=BEGIN,END,FAIL --mail-user=dje4001@med.cornell.edu --wrap="singularity exec ~/cloudreg.sif ./estrin_stitch.sh '$TMP' '$scratch_directory'"
