...
do
        ...
        do
        echo $channel
                sbatch --job-name=cellsegmentation --mem=40G --partition=sackler-gpu --gres=gpu:1 --mail-type=BEGIN,END,FAIL --mail-user=dje4001@med.cornell.edu --wrap="bash ./estrin_cellsegment.sh $channel $code_directory"
        done
done
exit
